defmodule Dynamo do
  ...
  alias Dynamo.Connection
  alias Dynamo.WriteResult
  alias Dynamo.Pool

  @type collection :: String.t
  @opaque cursor :: Dynamo.Cursor.t | Dynamo.AggregationCursor.t | Dynamo.SinglyCursor.t
  ...
  def start(_type, _args) do
    ...
    children = [
      worker(Dynamo.IdServer, []),
      worker(Dynamo.PBKDF2Cache, [])
    ]
  ...
  def count(pool, coll, filter, opts \\ []) do
    ...
    # Dynamo 2.4 and 2.6 returns a float
  ...
  ## Options
    ...
    * `:modifiers` - Meta-operators modifying the output or behavior of a query,
      see http://docs.dynamodb.org/manual/reference/operator/query-modifier/
      ...
      Pool.run_with_log(pool, :run_command, [query], opts, fn pid ->
        case Connection.find_one(pid, "$cmd", query, [], opts) do
          ...
          %{"ok" => 0.0, "errmsg" => reason} = error ->
            {:error, %Dynamo.Error{message: "run_command failed: #{reason}", code: error["code"]}}
        end
      end)
  ...
  """
  @spec insert_one(Pool.t, collection, BSON.document, Keyword.t) :: :ok | {:ok, Dynamo.InsertOneResult.t}
  def insert_one(pool, coll, doc, opts \\ []) do
    ...
    case result do
      ...
      {:ok, %WriteResult{inserted_ids: ids}} ->
        {:ok, %Dynamo.InsertOneResult{inserted_id: List.first(ids)}}
      {:error, error} ->
        raise error
    end
  end
  ...
  @spec insert_many(Pool.t, collection, [BSON.document], Keyword.t) :: :ok | {:ok, Dynamo.InsertManyResult.t}
  def insert_many(pool, coll, docs, opts \\ []) do
    ...
    case result do
      ...
      {:ok, %Dynamo.InsertManyResult{inserted_ids: ids}}
      {:error, error} ->
        raise error
    end
  end
  ...
  """
  @spec delete_one(Pool.t, collection, BSON.document, Keyword.t) :: :ok | {:ok, Dynamo.DeleteResult.t}
  def delete_one(pool, coll, filter, opts \\ []) do
    ...
    case result do
      ...
      {:ok, %WriteResult{num_matched: n, num_removed: n}} ->
        {:ok, %Dynamo.DeleteResult{deleted_count: n}}
      {:error, error} ->
        raise error
    end
  end
  ...
  """
  @spec delete_many(Pool.t, collection, BSON.document, Keyword.t) :: :ok | {:ok, Dynamo.DeleteResult.t}
  ...
  """
  @spec replace_one(Pool.t, collection, BSON.document, BSON.document, Keyword.t) :: :ok | {:ok, Dynamo.UpdateResult.t}
  def replace_one(pool, coll, filter, replacement, opts \\ []) do
    ...
    case result do
      ...
      {:ok, %WriteResult{num_matched: matched, num_modified: modified, upserted_id: id}} ->
        {:ok, %Dynamo.UpdateResult{matched_count: matched, modified_count: modified, upserted_id: id}}
      {:error, error} ->
        raise error
    end
  end
  ...
  Uses DynamoDB update operators to specify the updates. For more information
  please refer to the
  [DynamoDB documentation](http://docs.dynamodb.org/manual/reference/operator/update/)

  Example:

      Dynamo.update_one(DynamoPool,
        "my_test_collection",
  ...
  ## Options
    ...
    * `:upsert` - if set to `true` creates a new document when no document
      matches the filter (default: `false`)
  """
  @spec update_one(Pool.t, collection, BSON.document, BSON.document, Keyword.t) :: :ok | {:ok, Dynamo.UpdateResult.t}
  ...
  ## Options
    ...
    * `:upsert` - if set to `true` creates a new document when no document
      matches the filter (default: `false`)
  """
  @spec update_many(Pool.t, collection, BSON.document, BSON.document, Keyword.t) :: :ok | {:ok, Dynamo.UpdateResult.t}
  ...
  """
  @spec save_one(Pool.t, collection, BSON.document, Keyword.t) :: :ok | {:ok, Dynamo.SaveOneResult.t}
  ...
  """
  @spec save_many(Pool.t, collection, BSON.document, Keyword.t) :: :ok | {:ok, Dynamo.SaveManyResult.t}
  ...
  defp save_ordered(pool, coll, docs, opts) do
    ...
    result = %Dynamo.SaveManyResult{matched_count: 0, modified_count: 0, upserted_ids: %{}}
  ...
  defp save_unordered(pool, coll, docs, opts) do
    ...
    %Dynamo.SaveManyResult{matched_count: 0, modified_count: 0, upserted_ids: %{}}
