defmodule Dynamo.Connection.Utils do
  ...
  # for dynamosniff behind a flag
  if Mix.env in [:dev, :test] && System.get_env("DYNAMO_NO_BATCH_SEND") do
    def send(ops, s) do
      # Do a separate :gen_tcp.send/2 for each message because dynamosniff
      # cannot handle more than one message per packet. TCP is a stream
      # protocol, but no.
      # https://jira.dynamodb.org/browse/TOOLS-821
  ...
  def digest_password(username, password) do
    :crypto.hash(:md5, [username, ":dynamo:", password])
    |> Base.encode16(case: :lower)
  end
