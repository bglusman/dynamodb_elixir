defmodule Dynamo.Connection.Auth.SCRAM do
  @moduledoc false
  import Dynamo.BinaryUtils
  ...
  defp hi(password, salt, iterations) do
    Dynamo.PBKDF2Cache.pbkdf2(password, salt, iterations)
  end
  ...
  defp handle_error({:ok, %{"ok" => 0.0, "errmsg" => reason, "code" => code}}, username),
    do: {:error, %Dynamo.Error{message: "auth failed for '#{username}': #{reason}", code: code}}
end
