defmodule Dynamo.Connection do
  @moduledoc """
  A connection process to a DynamoDB server.
  """

  import Kernel, except: [send: 2]
  import Dynamo.Protocol
  import Dynamo.Connection.Utils
  require Logger
  alias Dynamo.Connection.Auth
  alias Dynamo.ReadResult
      ...
      {:ok, socket} ->
        ...
        case init_connection(s) do
          ...
          {:error, reason} ->
            {:stop, reason, s}
          {:tcp_error, reason} ->
            Logger.error "Dynamo tcp error (#{host}:#{port}): #{format_error(reason)}"
            {:backoff, s.opts[:backoff], s}
        end

      {:error, reason} ->
        Logger.error "Dynamo connect error (#{host}:#{port}): #{format_error(reason)}"
        {:backoff, s.opts[:backoff], s}
    end
  end
  ...
  defp init_connection(s) do
    # wire version
    # https://github.com/dynamodb/dynamo/blob/master/src/dynamo/db/wire_version.h
  ...
  def disconnect({:error, reason}, s) do
    ...
    Enum.each(s.queue, fn
      {_id, %{from: nil}} ->
        :ok
      {_id, %{from: from}} ->
        error = %Dynamo.Error{message: "Dynamo tcp error: #{formatted_reason}"}
        reply({:error, error}, from)
    end)

    host = s.opts[:hostname]
    port = s.opts[:port] || 27017
    Logger.error "Dynamo tcp error (#{host}:#{port}): #{formatted_reason}"
  ...
  defp message(:get_more, id, op_reply(flags: flags, docs: docs, cursor_id: cursor_id, from: from, num: num), s) do
    if :cursor_not_found in flags do
      reason = %Dynamo.Error{message: "cursor not found"}
  ...
  defp message(:get_last_error, id, op_reply(docs: [doc]), s) do
    case doc do
      ...
      %{"ok" => 1.0, "err" => message, "code" => code} ->
        ...
        # inserted, but dynamo always returns {n: 0}
        ...
        reply(id, {:error, %Dynamo.Error{message: message, code: code}}, s)
  ...
  defp failure(_command, id, op_reply(docs: [%{"$err" => reason, "code" => code }]), s) do
    reply(id, %Dynamo.Error{message: reason, code: code}, s)
  end
