defmodule Dynamo.ConnectionTest do
  use DynamoTest.Case, async: true
  alias Dynamo.Connection
  alias Dynamo.ReadResult, as: Read
  alias Dynamo.WriteResult, as: Write

  defp connect do
    assert {:ok, pid} =
           Connection.start_link(hostname: "localhost", database: "dynamodb_test")
    pid
  end
  ...
  defp connect_auth do
    assert {:ok, pid} =
           Connection.start_link(hostname: "localhost", database: "dynamodb_test",
                                 username: "dynamodb_user", password: "dynamodb_user")
    pid
  end
  ...
  test "auth wrong" do
    ...
    opts = [hostname: "localhost", database: "dynamodb_test",
            username: "dynamodb_user", password: "wrong"]

    capture_log fn ->
      assert {:ok, pid} = Connection.start_link(opts)
      assert_receive {:EXIT, ^pid, %Dynamo.Error{code: 18}}
    end
  end
  ...
  test "kill_cursors" do
    ...
    assert {:error, %Dynamo.Error{code: nil, message: "cursor not found"}} =
